...
class LoopPlayer {
  ...
  _ensureCtx() {
    if (!this.ctx) {
      ...
      // reverb: master -> dry -> out, master -> convolver -> wet -> out.
      // The dry path stays at 1; `reverb` (0..1) is the wet send level.
      this._dry = this.ctx.createGain();
      this._wet = this.ctx.createGain();
      this._wet.gain.value = this.reverb || 0;
      this._conv = this.ctx.createConvolver();
      this._conv.buffer = this._makeImpulse(2.8, 2.2);
      this.master.connect(this._dry);
      this._dry.connect(this.ctx.destination);
      this.master.connect(this._conv);
      this._conv.connect(this._wet);
      this._wet.connect(this.ctx.destination);
